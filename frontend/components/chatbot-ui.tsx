...
import { ArrowUp, LogOut, MessageCircle, Paperclip, Sparkles, UserRound } from "lucide-react"
...
export default function ChatbotUI() {
  ...
  useEffect(() => {
    ...
    api.getHistory().then(setMessages).catch(() => setAuthError("Could not load your conversation history.")).finally(() => setIsHistoryLoading(false))
  }, [authenticated])
      ...
      <main className="min-h-screen bg-[#090b0a] px-5 py-10 text-white">
        <div className="mx-auto flex min-h-[calc(100vh-5rem)] max-w-md items-center justify-center">
          <section className="w-full rounded-3xl border border-white/10 bg-white/[0.045] p-8 shadow-2xl shadow-black/30 backdrop-blur-xl">
            <div className="mb-8 flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-lime-400 text-black"><Sparkles size={21} /></div><div><p className="text-sm font-semibold tracking-[0.18em] text-lime-300 uppercase">Aryan AI</p><p className="text-xs text-white/45">Private inference workspace</p></div></div>
  ...
  return (
    <main className="min-h-screen bg-[#090b0a] px-4 py-4 text-white sm:px-6 sm:py-6">
      <div className="mx-auto flex min-h-[calc(100vh-2rem)] max-w-5xl flex-col overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.035] shadow-2xl shadow-black/40 backdrop-blur-xl sm:min-h-[calc(100vh-3rem)]">
        <header className="flex items-center justify-between border-b border-white/10 px-5 py-4 sm:px-7"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-xl bg-lime-400 text-black"><MessageCircle size={18} /></div><div><h1 className="font-semibold tracking-tight">Aryan AI</h1><p className="text-xs text-white/40">Fine-tuned assistant · online</p></div></div><button onClick={() => api.logout()} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-white/50 transition hover:bg-white/10 hover:text-white"><UserRound size={15} />{username}<LogOut size={15} /></button></header>
        <section className="flex flex-1 flex-col overflow-hidden"><div className="flex-1 overflow-y-auto px-5 py-7 sm:px-20 sm:py-10">{isHistoryLoading ? <div className="flex items-center justify-center py-20 text-sm text-white/40">Loading your history…</div> : messages.length === 0 ? <div className="mx-auto flex max-w-lg flex-col items-center justify-center py-20 text-center"><div className="mb-5 grid size-14 place-items-center rounded-2xl border border-lime-300/20 bg-lime-300/10 text-lime-300"><Sparkles /></div><h2 className="text-2xl font-semibold">What can I help with?</h2><p className="mt-2 text-sm text-white/45">Ask Aryan AI anything about projects, ideas, or technical work.</p></div> : <div className="mx-auto max-w-2xl space-y-6">{messages.map((message, index) => <div key={`${message.created_at}-${index}`} className={`flex gap-3 ${message.role === "user" ? "justify-end" : "justify-start"}`}><div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === "user" ? "bg-lime-400 text-black" : "border border-white/10 bg-white/[0.06] text-white/85"}`}><p className="whitespace-pre-wrap">{message.content}</p></div></div>)}{isLoading && <div className="flex items-center gap-2 text-sm text-white/45"><span className="size-2 animate-pulse rounded-full bg-lime-300" /><span className="size-2 animate-pulse rounded-full bg-lime-300 [animation-delay:150ms]" /><span className="size-2 animate-pulse rounded-full bg-lime-300 [animation-delay:300ms]" /> Aryan is thinking</div>}<div ref={messagesEndRef} /></div>}</div>
          <form onSubmit={sendMessage} className="mx-4 mb-4 rounded-2xl border border-white/10 bg-black/20 p-2 sm:mx-auto sm:mb-6 sm:w-[calc(100%-10rem)]"><textarea value={input} onChange={(event) => setInput(event.target.value)} onKeyDown={handleKeyDown} rows={2} placeholder="Message Aryan AI…" className="w-full resize-none bg-transparent px-3 py-2 text-sm outline-none placeholder:text-white/30" /><div className="flex items-center justify-between px-2 pb-1"><button type="button" aria-label="Attach a file" className="rounded-lg p-2 text-white/35 hover:bg-white/10 hover:text-white"><Paperclip size={17} /></button><div className="flex items-center gap-3"><span className="hidden text-xs text-white/30 sm:inline">Enter to send · Shift + Enter for newline</span><button type="submit" disabled={!input.trim() || isLoading} aria-label="Send message" className="grid size-9 place-items-center rounded-xl bg-lime-400 text-black transition hover:bg-lime-300 disabled:bg-white/10 disabled:text-white/25"><ArrowUp size={18} /></button></div></div></form>
        </section>
      </div>
    </main>
  )
